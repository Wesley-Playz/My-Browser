            ...
            webView.CoreWebView2InitializationCompleted += (s, e) =>
            {
                webView.CoreWebView2.ContextMenuRequested += async (sender, args) =>
                {
                    ...
                    try
                    {
                        ...
                        // Take Screenshot
                        var screenshot = new ToolStripMenuItem("Take Screenshot");
                        screenshot.Click += async (o, ev) =>
                        {
                            try
                            {
                                string script = @"
                                ...
                                string base64Image = await webView.CoreWebView2.ExecuteScriptAsync(script);
                                base64Image = base64Image.Replace("\"", "").Replace("\\u003d", "="); // Clean up result
                                ...
                                })
                                {
                                    ...
                                    {
                                        byte[] imageBytes = Convert.FromBase64String(base64Image.Substring("data:image/png;base64,".Length));
                                        await File.WriteAllBytesAsync(saveDialog.FileName, imageBytes);
                                        MessageBox.Show($"Screenshot saved to {saveDialog.FileName}");
                                    }
                                }
                            }
                            catch (Exception ex)
